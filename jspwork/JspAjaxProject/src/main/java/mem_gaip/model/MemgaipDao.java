...
public class MemgaipDao {
	...
	//id를 보내서 id가 존재하면 true를 리턴해준다.
	public boolean isIdcheck(String m_id) {
		
		boolean b=false; //있을 경우에만 true로 변경예정
	...
	public List<MemgaipDto> getAllDatas() {
		...
		String sql="select * from memgaip order by m_id desc";
	...
	public boolean isEqualPass(String m_num,String m_pass)
	{
		...
		String sql="select * from memgaip where m_num=? and m_pass=?";
		try {
			pstmt=conn.prepareStatement(sql);
			pstmt.setString(1,m_num);

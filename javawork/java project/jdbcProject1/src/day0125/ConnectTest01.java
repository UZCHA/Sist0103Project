...
public class ConnectTest01 {
	...
	public void connectSawon()
	{
		...
		try {
			...
			//여러줄이 가져올때는 while문으로 가져온다
			while (rs.next())//rs.next는 다음 데이터로 이동하면서 true반환, 더이상 데이터가 없으면 false반환
			{
				//db로부터 데이타를 가져오기
	...
	public void sawonRead()
	{
		...
		//count(*), round(avg(pay),0)의 경우 cnt나 pay같은 alias같은 별칭을 주어서 넣기 편하게 한다.
	...
	public static void main(String[] args) {
		...
		ConnectTest01 ct=new ConnectTest01();
		//ct.connectShop();
		ct.sawonRead();
